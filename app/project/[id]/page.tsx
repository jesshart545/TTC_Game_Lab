...
export default function ProjectWorkspace() {
  ...
  return <main className="workspace-page">
    ...
    <div className="workspace-grid">
      ...
      <aside className="assets-panel"><div className="assets-head"><div><small>PROJECT</small><h2>Details</h2></div><button type="button" onClick={() => fileInputRef.current?.click()}>＋</button></div><div className="detail-block"><span>PROJECT URL</span><code>{projectUrl}</code><Link href={projectUrl}>Open project ↗</Link></div><div className="detail-block"><span>GAME TOOLS</span><div className="tool-library">{TOOL_LIBRARY.map(tool=><div className="tool-library-row" key={tool.type}><div><b>{tool.name}</b><small>{tool.description}</small></div><button className="outline-btn" onClick={()=>addGameTool(tool.type)}>＋ Add</button></div>)}</div><div className="tool-config-list">{(project.gameTools || []).map(tool=><div className="tool-config" key={tool.id}><div><b>{tool.name}</b><em>{tool.type}</em></div><button className="outline-btn" onClick={()=>triggerGameTool(tool)}>Trigger</button></div>)}</div>{(project.gameTools || []).some(t=>t.name === "Trivia Board") && <div className="trivia-dashboard"><strong>TRIVIA BOARD</strong><small>Choose a question to fill the live overlay. Every question is source-backed; regenerate topics or questions whenever you want.</small><div className="trivia-actions"><button className="outline-btn" disabled={triviaBusy} onClick={()=>regenerateTrivia("generate")}>↻ Regenerate sourced board</button><button className="outline-btn" disabled={triviaBusy} onClick={()=>regenerateTrivia("source")}>↻ Refresh sources</button></div><div className="trivia-topic-editor"><small>TOPICS</small>{(triviaConfig || TRIVIA_CONFIG).categories.map((cat:any,i:number)=><input key={i} value={triviaTopics[i] ?? cat.name} onChange={e=>setTriviaTopics(v=>v.map((x,idx)=>idx===i?e.target.value:x))} placeholder={`Category ${i+1}`}/>)}</div><div className="trivia-grid">{(triviaConfig || TRIVIA_CONFIG).categories.map((cat:any,ci:number)=><div className="trivia-column" key={cat.name}><div className="trivia-column-head"><b>{cat.name}</b><button onClick={()=>regenerateTrivia("generate",cat.name)} disabled={triviaBusy}>↻</button></div>{cat.questions.map((q:any,qi:number)=><div className="trivia-cell" key={q.value}><button className={q.used ? "trivia-used" : ""} disabled={q.used} onClick={()=>triggerTriviaQuestion(ci,qi)}>{q.used ? "USED" : `${q.value}`}</button>{!q.used && <button className="trivia-reveal-btn" onClick={()=>revealTriviaAnswer(ci,qi)}>Reveal</button>}</div>)}</div>)}</div><button className="outline-btn" onClick={closeTriviaQuestion}>Close current clue</button></div></div></div><div className="detail-block"><span>WHEEL SETTINGS</span>{project.wheel?.enabled ? <><input className="wheel-input" value={project.wheel.title} onChange={e=>updateWheelField("title",e.target.value)} placeholder="Wheel title"/><input className="wheel-input" value={project.wheel.segments.join(", ")} onChange={e=>updateWheelField("segments",e.target.value)} placeholder="Prize, Challenge, Bonus"/><button className="outline-btn" onClick={spinWheel}>↻ Trigger wheel</button></> : <p className="empty-note">Add the Game Wheel from the library when you need it.</p>}</div><div className="detail-block"><span>ASSETS</span>{project.assets.map(a=><div className="asset-card" key={a.name}><div className="asset-card-title"><b>{a.name}</b><em>{a.type}</em></div>{renderAsset(a)}</div>)}{project.assets.length===0&&<p className="empty-note">No assets yet. Upload or generate them from the conversation.</p>}</div><div className="detail-block"><span>NEW PROJECT</span><button className="outline-btn" onClick={beginBlank}>＋ Start fresh</button></div></aside>
    </div>
  </main>;
}
