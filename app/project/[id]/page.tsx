...
export default function ProjectWorkspace() {
  ...
  function handleFiles(event: ChangeEvent<HTMLInputElement>) {
    ...
    files.forEach((file) => {
      ...
      reader.onload = () => {
        ...
        persist({ ...project, assets: [...project.assets, asset], updatedAt: "just now" });
  ...
  async function generateAsset(type: GeneratorType) {
    ...
    try {
      ...
      const asset: ProjectAsset = { name: `${type[0].toUpperCase()}${type.slice(1)} ${project.assets.length + 1}`, type: data.model || type, url };
      persist({ ...project, assets: [...project.assets, asset], updatedAt: "just now" });
      setAssetStatus(data.status === "pending" ? `${asset.name} started — generation is processing.` : `${asset.name} generated`);
    } catch (error) {
