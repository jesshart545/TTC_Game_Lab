...
export default function OverlayRuntime() {
  ...
  useEffect(() => { const all = loadProjects(); const p = all.find(x => x.slug === slug) || all[0]; setProject(p); if (!p) return; const bc = new BroadcastChannel(`ttc-project-${p.id}`); bc.onmessage = e => { if (e.data?.type === "PROJECT_EVENT") { setEvent(e.data.action); setTimeout(() => setEvent(""), 900); } }; return () => bc.close(); }, [slug]);
