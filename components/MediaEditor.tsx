...
export default function MediaEditor({asset,onSave,onSaveAsNew,onClose}:{asset:ProjectAsset;onSave:(asset:ProjectAsset)=>void;onSaveAsNew?:(asset:ProjectAsset)=>Promise<void>;onClose:()=>void}) {
  ...
  async function runAiEdit() {
    ...
    try {
      ...
      if (!data.url) throw new Error("Nano Banana 2 returned no edited image.");
  ...
  return <div className="media-editor-backdrop" role="dialog" aria-modal="true"><div className="media-editor-modal">
    ...
    <div className="media-editor-fields">
      ...
      <div className="media-editor-ai"><span>AI EDIT</span><div><input value={aiPrompt} onChange={e=>setAiPrompt(e.target.value)} placeholder="Remove background, make it haunted, change shirt color…" onKeyDown={e=>{if(e.key==="Enter")void runAiEdit()}}/><button type="button" disabled={!aiPrompt.trim()||aiEditing} onClick={()=>void runAiEdit()}>{aiEditing?"Editing…":"AI Edit"}</button></div><small>Uses Nano Banana 2 through fal.ai and saves the result as a new asset.</small>{aiError&&<small className="media-editor-error">{aiError}</small>}</div></>}
    ...
    <div className="media-editor-actions"><button className="outline-btn" onClick={reset}>Reset</button><button className="outline-btn" onClick={onClose}>Cancel</button>{onSaveAsNew&&<button className="outline-btn" disabled={savingNew||(video&&effectiveEnd<=trimStart)} onClick={async()=>{setSavingNew(true);try{await onSaveAsNew(editedAsset());onClose();}finally{setSavingNew(false)}}}>{savingNew?(video?"Trimming…":"Rendering…"):(video?"Save Trimmed Video":"Save as New Asset")}</button>}<button className="build-btn" onClick={apply}>Save Edit Settings</button></div>
  </div></div>;
}
